CLASS lsc_ZI_M_PO_ITEM DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_ZI_M_PO_ITEM IMPLEMENTATION.

  METHOD save_modified.
    " Kiểm tra có DataFile nào được update messagetype = 'J' không
    IF update-datafilepoitem IS NOT INITIAL.

      " Tìm records có messagetype = 'J'
      LOOP AT update-datafilepoitem ASSIGNING FIELD-SYMBOL(<upd>)
        WHERE messagetype = 'J'.
      ...
      IF lv_found = abap_true.
        ...
        DATA(lv_job_text) = CONV cl_apj_rt_api=>ty_job_text( update-datafilepoitem[ 1 ]-Message ).
        GET TIME STAMP FIELD DATA(ls_ts).

        TRY.
            cl_apj_rt_api=>schedule_job(
              EXPORTING
                iv_job_template_name   = 'ZJT_PO_ITEM'
...
CLASS lhc_ManageFilePOItem DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    CONSTANTS:
    ...
    METHODS get_global_authorizations FOR GLOBAL AUTHORIZATION
      IMPORTING REQUEST requested_authorizations FOR ManageFilePOItem RESULT result.

    METHODS earlynumbering_create FOR NUMBERING
      IMPORTING entities FOR CREATE ManageFilePOItem.

    METHODS downloadFileItem FOR MODIFY
      IMPORTING keys FOR ACTION ManageFilePOItem~downloadFileItem RESULT result.

    METHODS downloadTemplateItem FOR MODIFY
      IMPORTING keys FOR ACTION ManageFilePOItem~downloadTemplateItem.

    METHODS uploadExcelItem FOR MODIFY
      IMPORTING keys FOR ACTION ManageFilePOItem~uploadExcelItem.

    METHODS setStatusToOpenItem FOR DETERMINE ON MODIFY
      IMPORTING keys FOR ManageFilePOItem~setStatusToOpenItem.

    METHODS getExcelDataItem FOR DETERMINE ON SAVE
      IMPORTING keys FOR ManageFilePOItem~getExcelDataItem.

ENDCLASS.

CLASS lhc_ManageFilePOItem IMPLEMENTATION.
  ...
  METHOD earlynumbering_create.
    LOOP AT entities
      ...
      APPEND CORRESPONDING #( <f_entities> ) TO mapped-managefilepoitem.
      ...
      TRY.
        ...
        CATCH cx_uuid_error.
          ...
          APPEND VALUE #( %cid      = <f_entities>-%cid
                 ...
                 TO reported-managefilepoitem.

          APPEND VALUE #( %cid      = <f_entities>-%cid
                          %key      = <f_entities>-%key
                          %is_draft = <f_entities>-%is_draft )
                 TO failed-managefilepoitem.
  ...
  METHOD uploadExcelItem.
    DATA lt_file TYPE STANDARD TABLE OF ty_file_upload.

    DATA: lt_mn_file TYPE TABLE FOR CREATE zi_m_po_item,
          ls_mn_file LIKE LINE OF lt_mn_file,

          lt_file_c  TYPE TABLE FOR CREATE zi_m_po_item\_datafile,
          ls_file_c  LIKE LINE OF lt_file_c.

    DATA: lt_keys TYPE TABLE FOR READ IMPORT zi_m_po_item.
    ...
    IF lt_mn_file IS NOT INITIAL.
      MODIFY ENTITIES OF zi_m_po_item IN LOCAL MODE
        ENTITY ManageFilePOItem
  ...
  METHOD setStatusToOpenItem.
    READ ENTITIES OF zi_m_po_item IN LOCAL MODE
     ENTITY ManageFilePOItem
       FIELDS ( status )
  ...
  METHOD getExcelDataItem.
    DATA: lt_file TYPE STANDARD TABLE OF ty_file_upload.

    DATA: lt_file_c TYPE TABLE FOR CREATE zi_m_po_item\\ManageFilePOItem\_datafile,
          ls_file_c LIKE LINE OF lt_file_c.
    ...
    " ── 3. Process data Raw ───────────────────────────────────────
    DATA: lt_data_file TYPE TABLE OF zi_d_po_item.
    ...
    LOOP AT lt_file INTO DATA(ls_file).
      ...
      <lfs_data_file>-AccountAssignmentCategory = ls_file-AccountAssignmentCategory.
      <lfs_data_file>-PurchaseOrderItemCategory = ls_file-PurchaseOrderItemCategory.
      <lfs_data_file>-PurchaseRequisition       = |{ ls_file-PurchaseRequisition ALPHA = IN }|.
      <lfs_data_file>-purchaserequisitionitem   = |{ ls_file-purchaserequisitionitem ALPHA = IN }|.
      ...
      <lfs_data_file>-OrderId                   = |{ ls_file-OrderId ALPHA = IN }|.
      <lfs_data_file>-OrderInternalID           = |{ ls_file-OrderInternalID ALPHA = IN }|.
      <lfs_data_file>-FunctionalArea            = |{ ls_file-FunctionalArea ALPHA = IN }|.
    ENDLOOP.
    ...
    LOOP AT lt_data_file INTO DATA(ls_seen_dup).
      DATA(lv_key) =
        ...
        && ls_seen_dup-AccountAssignmentCategory
        && ls_seen_dup-PurchaseOrderItemCategory
        && ls_seen_dup-PurchaseRequisition
        && ls_seen_dup-purchaserequisitionitem
        ...
        && ls_seen_dup-OrderId
        && ls_seen_dup-OrderInternalID
        && ls_seen_dup-FunctionalArea.
    ...
    LOOP AT lt_data_file INTO DATA(ls_data_file).
      ...
      TRY.
          ls_file_c = VALUE #(
              ...
              AccountAssignmentCategory = ls_data_file-AccountAssignmentCategory
              PurchaseOrderItemCategory = ls_data_file-PurchaseOrderItemCategory
              PurchaseRequisition       = ls_data_file-PurchaseRequisition
              PurchaseRequisitionItem   = ls_data_file-PurchaseRequisitionItem
              Material                  = ls_data_file-Material
              PurchaseOrderItemText     = ls_data_file-PurchaseOrderItemText
              ...
              OrderID                   = ls_data_file-OrderID
              OrderInternalID           = ls_data_file-OrderInternalID
              FunctionalArea            = ls_data_file-FunctionalArea
...
CLASS lhc_DataFilePOItem DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    CONSTANTS:
      BEGIN OF file_status,
        open      TYPE c LENGTH 1 VALUE 'M', "Not process
    ...
    METHODS get_instance_features FOR INSTANCE FEATURES
      IMPORTING keys REQUEST requested_features FOR DataFilePOItem RESULT result.

    METHODS get_global_authorizations FOR GLOBAL AUTHORIZATION
      IMPORTING REQUEST requested_authorizations FOR DataFilePOItem RESULT result.

    METHODS PostConfirm FOR MODIFY
      IMPORTING keys FOR ACTION DataFilePOItem~PostConfirm RESULT result.

    METHODS setJob FOR MODIFY
      IMPORTING keys FOR ACTION DataFilePOItem~setJob RESULT result.

    METHODS setStatusToUpdate FOR DETERMINE ON MODIFY
      IMPORTING keys FOR DataFilePOItem~setStatusToUpdate.

ENDCLASS.

CLASS lhc_DataFilePOItem IMPLEMENTATION.
  ...
  METHOD PostConfirm.
    ...
    "── 2. Process API ─────────────────────────────────────────────────
    zcl_po_item_implement=>processing_api( CHANGING ct_data = lt_data ).

    " ── 3. Update status ─────────────────────────────────────
    MODIFY ENTITIES OF zi_m_po_item IN LOCAL MODE
      ENTITY DataFilePOItem
